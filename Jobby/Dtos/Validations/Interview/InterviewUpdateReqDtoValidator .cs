...
{
    public class InterviewUpdateReqDtoValidator : AbstractValidator<InterviewUpdateReqDto>
    {
        public InterviewUpdateReqDtoValidator()
        {
            RuleFor(x => x.Stage)
                .IsInEnum();
        ...
        private static bool BeInFutureOrNow(DateTimeOffset dt)
            => dt >= DateTimeOffset.UtcNow.AddMinutes(-1);
    }
}
